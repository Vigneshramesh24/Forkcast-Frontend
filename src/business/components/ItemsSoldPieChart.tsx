...
const ItemsSoldPieChart: React.FC = () => {
  const { itemsSoldAggregate } = useBusinessData();
  const [mode, setMode] = useState<'quantity'|'revenue'>('quantity');
  const raw = itemsSoldAggregate();
  const data = raw
      ...
      <CardHeader className="pb-3 border-b border-border/50">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base font-semibold flex items-center gap-2">
            ...
            Items {mode==='quantity' ? 'Sold' : 'Revenue'} Breakdown
          </CardTitle>
        ...
        <div className="h-64">
          {data.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data} dataKey={mode} nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
